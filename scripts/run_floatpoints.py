...
def run_synthesis(percision_level = 24, f128 = False):
    ...
    print(f"Compiling PRECISION_LEVEL={percision_level} {'f128' if f128 else 'f64'}", end=' ', flush=True)
    os.environ["PRECISION_LEVEL"] = str(percision_level)
    process = subprocess.run(commands, stdout=subprocess.PIPE, stderr=subprocess.PIPE, text=True)
    del os.environ["PRECISION_LEVEL"]
...
if __name__ == "__main__":
    store = pd.HDFStore('.cache/fixed_point_results.h5')
    if 'df' in store:
        df = store['df']
    else:
        data = []
